...
int ** psuedo_prod_matrix(int n, int l, int m, int ** a, int ** b);
int ** trans_matrix(int n, int m, int ** a);

// MPI functions
int MPI_Prod_matrix(int n, int ** a, int ** b, int ** c, int root, MPI_Comm comm);
int MPI_Prod_matrix_rows(int n, int ** a, int ** b, int ** c, int root, MPI_Comm comm);
...
int MPI_Prod_matrix(int n, int ** a, int ** b, int ** c, int root, MPI_Comm comm)
{
	...
	int error = MPI_Scatter(a[0], n*n/size, MPI_INT,  local_a[0], n*n/size, MPI_INT, root, comm);
	if(error != MPI_SUCCESS) return error;


	error = MPI_Bcast(b[0], n*n, MPI_INT, root, comm);
	if(error != MPI_SUCCESS) return error;

	// calculate local_c = local_a * b

	local_c = psuedo_prod_matrix(n/size, n, n, local_a, trans_matrix(n,n,b));
	
	// gather local_c
	error = MPI_Gather(local_c[0], n*n/size, MPI_INT, c[0], n*n/size, MPI_INT, root, comm);
	if(error != MPI_SUCCESS) return error;
...
int MPI_Prod_matrix_rows(int n, int ** a, int ** b, int ** c, int root, MPI_Comm comm)
{
	...
	// scatter a to local_a and bcast b
	int error = MPI_Scatter(a[0], n/size, rowtype,  local_a[0], n/size, rowtype, root, comm);
	if(error != MPI_SUCCESS) return error;


	error = MPI_Bcast(b[0], n, rowtype, root, comm);
	if(error != MPI_SUCCESS) return error;
	...
	// gather local_c
	error = MPI_Gather(local_c[0], n/size, rowtype, c[0], n/size, rowtype, root, comm);
	if(error != MPI_SUCCESS) return error;
