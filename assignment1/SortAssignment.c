...
int MPI_Shell_sort(int n, double* array, int root, MPI_Comm comm);

// Bucket Sort
int  MPI_Sort_bucket(int n, double* a, double max, int root, MPI_Comm comm);

// Direct Sort
int MPI_Direct_mergesort(int n, double* array, int root, MPI_Comm comm);
...
int main(int argc, char** argv) {
    ...
    double* oddEven_array, * shell_array, * bucket_array, *direct_array;

    //allocate space for an array of doubles, size n
    oddEven_array = (double*)calloc(n, sizeof(double));
    shell_array = (double*)calloc(n, sizeof(double));
    bucket_array = (double*)calloc(n, sizeof(double));
    direct_array = (double*)calloc(n, sizeof(double));
    ...
    if (rank == 0) {
        ...
        for (i = 0; i < n; i++)
        {
            double elem = ((double)rand() / RAND_MAX) * m;
            oddEven_array[i] = elem;
            shell_array[i] = elem;
            bucket_array[i] = elem;
            direct_array[i] = elem;
        }
    ...
    //Odd Even Sort
    result = MPI_Sort_oddeven(n, oddEven_array, 0, MPI_COMM_WORLD);
    if (result != MPI_SUCCESS) {
        return result;
    }
  
    // wait for all processors then print newline
    MPI_Barrier(MPI_COMM_WORLD);

    // Shell Sort
    result = MPI_Shell_sort(n, shell_array, 0, MPI_COMM_WORLD);
    if (result != MPI_SUCCESS) {
        return result;
    }
    ...
    // Bucket Sort
    result = MPI_Sort_bucket(n, bucket_array, m, 0, MPI_COMM_WORLD);
    if (result != MPI_SUCCESS) {
        return result;
    }
    ...
    // Direct Sort
    result = MPI_Direct_mergesort(n, direct_array, 0, MPI_COMM_WORLD);
    if (result != MPI_SUCCESS) {
        return result;
    }
...
int MPI_Direct_mergesort(int n, double* array, int root, MPI_Comm comm){	
	...
	// scatter array to local_a with double elements
    time = MPI_Wtime();
	error = MPI_Scatter(array, n/size, MPI_DOUBLE, local_a, n/size, MPI_DOUBLE, root, comm);
	...
	// gather local_a to array with double elements
    time = MPI_Wtime();
    ...
    printf("P:%d Direct Sort commT = %lf compT = %lf\n", rank, commT, compT);
	return MPI_SUCCESS;
...
int MPI_Sort_bucket(int n, double* a, double max, int root, MPI_Comm comm)
{
    ...
    printf("P:%d Bucket Sort commT = %lf compT = %lf\n", rank, commT, compT);
...
int MPI_Shell_sort(int n, double* array, int root, MPI_Comm comm) {
    ...
    while (left < right) {
        ...
        if (pair > rank) MPI_Exchange(n / size, local_a, rank, pair, comm);
        else if (pair < rank) MPI_Exchange(n / size, local_a, pair, rank, comm);

        // halve the shell
        int middle = (left + right) / 2;
        if (rank <= middle)right = middle;
        else left = middle + 1;
    }
    time = MPI_Wtime() - time;
    commT += time;
    ...
    for (int i = 0; i < size; i++) {
        if ((i + rank) % 2 == 0) {
            if (rank < size - 1)MPI_Exchange(n / size, local_a, rank, rank + 1, comm);

        }
        else {
            if (rank > 0)MPI_Exchange(n / size, local_a, rank - 1, rank, comm);
        }
        MPI_Barrier(comm);
        // test isSorted
        int answer;
        ...
        if (answer == 1) {
            //if (rank == 0)printf("--Shell finished in %d iterations\n", i + 1);
            break;
        }
    }
    time = MPI_Wtime() - time;
    ...
    printf("P:%d Shell Sort commT = %lf compT = %lf\n", rank, commT, compT);
...
int MPI_Sort_oddeven( int n, double * array, int root, MPI_Comm comm ) {
    ...
    for(int i=0;i<size;i++){
        ...
        MPI_Is_sorted(n/size, local_a, &answer, root, comm);
        if(answer == 1){
            //if(rank==0)printf("--Oddeven finished in %d iterations\n", i+1);
            break;
        }
    }
    time = MPI_Wtime() - time;
    ...
    printf("P:%d Odd Even Sort commT = %lf compT = %lf\n", rank, commT, compT);
    return MPI_SUCCESS;
}
